...
func NewSignerSender(privateKeyHex string, clientURI string, clientTimeoutS uint) (*SignerSender, error) {
	...
	clientInstance := client.NewGrpcClientWithTimeout(
		clientURI,
		time.Duration(clientTimeoutS)*time.Second,
	)
